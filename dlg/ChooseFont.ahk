...
ChooseFont(Owner := 0, FontName := 'Arial', Options := '', Flags := 0x10100)
{
    ...
    Local CHOOSEFONT
    NumPut(VarSetCapacity(CHOOSEFONT, A_PtrSize = 8 ? 104 : 60, 0), &CHOOSEFONT, 'UInt')
    NumPut(Owner                                                              , &CHOOSEFONT+A_PtrSize      , 'Ptr' )
    NumPut(&LOGFONT                                                           , &CHOOSEFONT+3*A_PtrSize    , 'UPtr')
    NumPut(Flags                                                              , &CHOOSEFONT+4*A_PtrSize + 4, 'UInt')
    NumPut(((Color & 255) << 16) | (((Color >> 8) & 255) << 8) | (Color >> 16), &CHOOSEFONT+4*A_PtrSize + 8, 'UInt')

    If (DllCall('comdlg32.dll\ChooseFontW', 'UPtr', &CHOOSEFONT))
    {
        Local Font      := {}
